...
from atis_templatize import process, deanonymize
...
class Interactive:
  def __init__(self, dropout, num_folds, seq_length, seed, epochs, embedding_file, skip_first, rnn_size, word_vec_size, nl_file, sql_file, plot_file, batch_size, grammar_file, 
      data_dir, models_dir, db, schema, ppscale, ppfile, host, user, passwd, alignments):
    self.host = host
    self.user = user
    self.passwd = passwd
    ...
    self.alignments = []
    for line in open(alignments, 'r'):
      (src, tgt) = line.strip().split('\t')
      self.alignments.append((src, tgt))
  ...
  def getData(self, nl_file, sql_file):
    ...
    for nl, sql in itertools.izip(open(nl_file, 'r'), open(sql_file, 'r')):
      (anon_nl, t_sql, map_nl) = process(nl.strip(), sql.strip(), self.alignments)
  ...
  def ingestFoldBatch(self, batch, prefix):
    for i in self.train[batch]:
      ...
      (t_nl, t_sql, m_nl) = process(nl.strip(), sql.strip(), self.alignments)
  ...
  def testNeuralModel(self, prefix, batch):
    ...
    for i in self.train[batch]:
      ...
      predPtr += 1
      (corr, ran, err, num, gold, q_ret) = compute(self.db, 1, gold=sql, q1=p_sql, host=self.host, user=self.user, passwd=self.passwd)
  ...
  def trainNeuralModel(self, prefix):
    corpus = generateFromList(self.grammar, self.schema, self.db, self.ppscale, self.ppfile, '../data/ppdb/stopwords.txt', self.host, self.user, self.passwd)
...
if __name__ == '__main__':
  ...
  gflags.DEFINE_string("train_nl", "../data/atis/train_dev.nl", "Training File Nl")
  gflags.DEFINE_string("train_sql", "../data/atis/train_dev.sql", "Training File Sql")
  ...
  gflags.DEFINE_string("schema", "../data/atis/atis.schema", "grammar file")
  gflags.DEFINE_string("data_dir", "../data/atis/tmp/", "Directory for temporary data")
  gflags.DEFINE_string("models_dir", "../data/atis/tmp/models", "Directory for temporary models")
  gflags.DEFINE_string("db", "atis", "database to run queries")
  gflags.DEFINE_string("host", "atis", "database to run queries")
  gflags.DEFINE_string("user", "atis", "database to run queries")
  gflags.DEFINE_string("passwd", "atis", "database to run queries")
  gflags.DEFINE_integer("batch_size", 406, "batch size")
  ...
  gflags.DEFINE_integer("seq_length", 150, "max sequence length")
