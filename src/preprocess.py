...
class Indexer:
    def __init__(self, args, symbols = ["<blank>","<unk>","<s>","</s>"]):
    ...
    def remove_unks(self, ls):
      i = 0
      while i < len(ls):
        if ls[i] not in self.d:
          del ls[i]
        else:
          i += 1

    def clean(self, line):
        line = line.replace(self.PAD, "")
        line = line.replace(self.BOS, "")
        line = line.replace(self.EOS, "")

        if self.args.strip_roles == 1:
          line = re.sub(":arg[012]( |$)", "", line.strip())
          line = re.sub(":ARG[012]( |$)", "", line.strip())
          line = re.sub(":op[0-9]*( |$)", "", line.strip())
          line = re.sub(":name( |$)", "", line.strip())
          line = re.sub(":snt[0-9]*( |$)", "", line.strip())
          line = re.sub(":quant( |$)", "", line.strip())
          line = re.sub(":unit( |$)", "", line.strip())
          line = re.sub(":value( |$)", "", line.strip())
          line = re.sub(":year( |$)", "", line.strip())
          line = re.sub(":domain-of( |$)", "", line.strip())
          line = re.sub("\S+-quantity( |$)", "", line.strip())
          line = re.sub("\S+-entity( |$)", "", line.strip())
        return line
    ...
    def prune_vocab_by_freq(self, f):
        vocab_list = [(word, count) for word, count in self.vocab.iteritems()]
        for (word, freq) in vocab_list:
            if word not in self.d and freq > f:
                self.d[word] = len(self.d) + 1
...
def get_data(args):
    src_indexer = Indexer(args, ["<blank>","<unk>","<s>","</s>"])
    target_indexer = Indexer(args, ["<blank>","<unk>","<s>","</s>"])    
    char_indexer = Indexer(args, ["<blank>","<unk>","{","}"])
    char_indexer.add_w([src_indexer.PAD, src_indexer.UNK, src_indexer.BOS, src_indexer.EOS])
    word2vec_indexer = Indexer(args, ["<blank>","<unk>","<s>","</s>"])

    def make_vocab(srcfile, targetfile, seqlength, max_word_l=0, chars=0, modify=True):
        num_sents = 0
        for _, (src_orig, targ_orig) in \
            ...
            src_word2vec = src_orig.strip().split()
    ...
    def convert(srcfile, targetfile, alignments_file, batchsize, seqlength, outfile, num_sents,
        ...
        sources_word2vec = np.zeros((num_sents, newseqlength), dtype=int)
        source_lengths = np.zeros((num_sents,), dtype=int)
        target_lengths = np.zeros((num_sents,), dtype=int)
#         alignments = np.zeros((num_sents, newseqlength, newseqlength), dtype=int)
        if chars==1:
        ...
        align_f = None if alignments_file is None else open(alignments_file, 'r')
        count = 0
        for _, (src_orig, targ_orig) in \
                enumerate(itertools.izip(open(srcfile,'r'), open(targetfile,'r'))):
            ...
            src_word2vec =  [src_indexer.BOS] + src_orig.strip().split() + [src_indexer.EOS]
            max_sent_l = max(len(targ), len(src), max_sent_l)
            align_orig = None
            if align_f is not None:
              align_orig = align_f.readline().strip().split()

            if len(targ) > newseqlength or len(src) > newseqlength or len(targ) < 3 or len(src) < 3:
                dropped += 1
                continue                   

#             if align_orig is not None:
#               print(align_orig)
#               print(len(align_orig))
#               print(len(targ))
#               print(count)
#               for t in range(1, len(targ) - 1):
#                 for s in range(1, len(src) - 1):
#                   alignments[sent_id][t][s] = (1 if align_orig[t - 1] == (s - 1) else 0)
            ...
            src_word2vec = pad(src_word2vec, newseqlength, src_indexer.PAD)
            src_char = []
            for word in src:
                if chars == 1:
                    word = char_indexer.clean(word)
            ...
            src_word2vec = word2vec_indexer.convert_sequence(src_word2vec)

              
            src = np.array(src, dtype=int)
            src_word2vec = np.array(src_word2vec, dtype=int)
            ...
            sources_word2vec[sent_id] = np.array(src_word2vec, dtype=int)
        ...
        if shuffle == 1:
            ...
            sources_word2vec = sources_word2vec[rand_idx]
#             alignments = alignments[rand_idx]
        ...
        sources_word2vec = sources_word2vec[source_sort]
...
#         alignments = alignments[source_sort]
        ...
        f["source"] = sources
        f["source_word2vec"] = sources_word2vec
        f["target"] = targets
#         f["alignments"] = alignments
        ...
        if chars == 1:            
            del sources, sources_word2vec, targets, target_output
    ...
    max_word_l, num_sents_train = make_vocab(args.srcfile, args.targetfile,
                                             args.seqlength, 0, args.chars, True)
    ...
    max_word_l, num_sents_valid = make_vocab(args.srcvalfile, args.targetvalfile,
                                             args.seqlength, max_word_l, args.chars, False)
    ...
    if args.vocab_unk_threshold > 0:
      src_indexer.prune_vocab_by_freq(args.vocab_unk_threshold)
      target_indexer.prune_vocab_by_freq(args.vocab_unk_threshold)
    else:
    ...
    if args.word2vecfile != '':
        print('Loading pre-specified word2vec vocab from ' + args.word2vecfile)
        word2vec_indexer.load_vocab(args.word2vecfile, 0)
    ...
    max_sent_l = 0
    max_sent_l = convert(args.srcvalfile, args.targetvalfile, None, args.batchsize, args.seqlength,
                         args.outputfile + "-val.hdf5", num_sents_valid,
                         max_word_l, max_sent_l, args.chars, args.unkfilter, args.shuffle)
    max_sent_l = convert(args.srcfile, args.targetfile, args.alignments_file, args.batchsize, args.seqlength,
...
def main(arguments):
    ...
    parser.add_argument('--vocab_unk_threshold', help="Threshold for src and target unks",
                                                type=int, default=0)
    ...
    parser.add_argument('--targetfile', help="Path to target training data, "
                                           "where each line represents a single "
                                           "source/target sequence.", required=True)
    parser.add_argument('--alignments_file', help="Path to training alignments file")
    ...
    parser.add_argument('--charvocabfile', help="If working with a preset vocab, "
                                         "then including this use the char vocab provided here.",
                                          type = str, default='')    
    parser.add_argument('--word2vecfile', help="word2vec dictionary file",
                                          type = str, default='')
    ...
    parser.add_argument('--shuffle', help="If = 1, shuffle sentences before sorting (based on  "
                                           "source length).",
                                          type = int, default = 0)
    parser.add_argument('--strip_roles', help="If = 1, strip amr roles"
                                           "source length).",
                                          type = int, default = 0)
