...
if [ "$1" == "test" ]; then
	...
	python anonymize.py --schema ${SCHEMA} --sqlfile ${OUT_DIR}/test.pred.txt --mapfile ${OUT_DIR}/${TEST_NL}.tem.map --reverse --stop_file ../data/ppdb/stopwords.txt
...
do
	...
	for ((i=1; i<=${EPOCHS}; i++)); do
	  ...
	  python anonymize.py --schema ${SCHEMA} --sqlfile ${MODELS_DIR}/preds/${f}.pred.txt --mapfile ${OUT_DIR}/${DEV_NL}.tem.map --reverse --stop_file ../data/ppdb/stopwords.txt
