...
if [ "$1" == "test" ]; then
	...
	python atis_templatize.py --mapfile ${OUT_DIR}/${TEST_NL}.tem.map --sqlfile ${OUT_DIR}/test.pred.txt --inst deanonymize
...
python atis_templatize.py --nlfile ${OUT_DIR}/${TRAIN_NL} --sqlfile ${OUT_DIR}/${TRAIN_SQL}
python atis_templatize.py --nlfile ${OUT_DIR}/${DEV_NL} --sqlfile ${OUT_DIR}/${DEV_SQL}
python atis_templatize.py --nlfile ${OUT_DIR}/${TEST_NL} --sqlfile ${OUT_DIR}/${TEST_SQL}
...
do
  ...
  MODELS_DIR=${PREFIX}
    ...
    python atis_templatize.py --mapfile ${OUT_DIR}/${DEV_NL}.tem.map --sqlfile ${MODELS_DIR}/preds/${f}.pred.txt --inst deanonymize
