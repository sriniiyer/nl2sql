...
class Query:
  def __init__(self, t, q, db=None, schemaObj=None, host=None, user=None, passwd=None):
    self.SEP = '$' 
  ...
  def output(self, schema, db, paraphraser, data):
    ...
    for typ in sorted(schema.types, key=len, reverse=True):
      if typ == "INTEGER" or typ == "YEAR":
        ...
        sqlCopy = re.sub(typ + "@\d+", "\"placeholder\"", sqlCopy) 

    # try to run sqlCopy
    try:
      res1 = runQueryWithConn(sqlCopy, 3000, db, self.host, self.user, self.passwd)
